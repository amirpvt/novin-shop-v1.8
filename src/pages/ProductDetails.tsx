  ...
  if (!product) {
    return (
      <div className="min-h-screen bg-cream-50 pt-40 pb-20 flex flex-col items-center justify-center text-center px-4">
        ...
        <p className="text-stone-500 mt-2">ممکن است حذف شده باشد یا لینک اشتباه است</p>
    ...
    <div className="min-h-screen bg-cream-50 pt-10 lg:pt-6 pb-20 font-sans text-right" dir="rtl">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 text-sm font-bold text-stone-500 hover:text-paprika-700 mb-8"
        >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 bg-white rounded-[3rem] border border-stone-200 shadow-xl overflow-hidden p-8 sm:p-12">
          ...
          <div className="relative group">
            ...
            <div className="relative aspect-square overflow-hidden rounded-[2.5rem] border border-stone-100 shadow-inner bg-stone-50">
              <img
                src={product.image}
                alt={product.name}
                className={`h-full w-full object-cover transition-transform duration-700 group-hover:scale-105 ${isOutOfStock ? 'grayscale' : ''}`}
              />
            </div>
            <div className="absolute top-6 right-6 flex flex-col gap-2">
              {product.tag && (
                <span className="bg-paprika-600 text-white px-5 py-2 rounded-full text-xs font-black shadow-lg">
                  {product.tag === "best" ? "پرفروش" : product.tag === "new" ? "جدید" : product.tag}
                </span>
              )}
              {isOutOfStock && (
                <span className="bg-stone-800 text-white px-5 py-2 rounded-full text-xs font-black shadow-lg">ناموجود</span>
              )}
            </div>
          ...
          <div className="flex flex-col">
            <div className="mb-6">
              ...
              <p className="mt-3 text-stone-400 text-sm font-medium flex items-center gap-2">
                <span>{product.unit}</span>
                {product.stock !== undefined && <span>• {product.stock} عدد موجود</span>}
              </p>
            </div>
            ...
            <div className="flex items-center gap-4 mb-8 p-5 rounded-3xl bg-stone-50 border border-stone-100">
              <div>
                ...
                {product.is_featured && <span className="text-[10px] bg-emerald-100 text-emerald-700 px-2 py-1 rounded-full font-bold mt-2 inline-block">پیشنهاد ویژه</span>}
              </div>
              {isOutOfStock ? (
                ...
                <div className="mr-auto flex items-center gap-1.5 text-emerald-600 font-bold text-sm">
                  <span className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" />موجود در انبار
                </div>
              )}
            </div>
            ...
            <div className="space-y-4 flex-1 mb-8">
              ...
              <ul className="space-y-2.5 mt-6">
                ...
                <li className="flex items-center gap-2 text-sm"><CheckIcon className="h-4 w-4 text-emerald-500" />بدون مواد نگهدارنده غیرمجاز</li>
                <li className="flex items-center gap-2 text-sm"><CheckIcon className="h-4 w-4 text-emerald-500" />بسته‌بندی وکیوم بهداشتی</li>
                <li className="flex items-center gap-2 text-sm"><CheckIcon className="h-4 w-4 text-emerald-500" />ارسال با ماشین یخچال‌دار</li>
              </ul>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <button
                ...
                className={`flex items-center justify-center gap-2 py-4 rounded-2xl font-black text-lg shadow-lg transition active:scale-95 ${isOutOfStock ? "bg-stone-200 text-stone-400 cursor-not-allowed shadow-none" : "bg-paprika-600 text-white shadow-paprika-600/20 hover:bg-paprika-700"}`}
              >
                🛒 {isOutOfStock ? "ناموجود" : "افزودن به سبد"}
              </button>
              <button
                onClick={() => { addWholesaleItem(product); navigate("/wholesale"); }}
                disabled={isOutOfStock}
                className={`flex items-center justify-center gap-2 py-4 rounded-2xl font-bold text-lg border-2 transition ${isOutOfStock ? "border-stone-100 bg-stone-50 text-stone-300" : "border-stone-200 bg-white text-stone-700 hover:border-paprika-600 hover:text-paprika-700"}`}
              >
                📦 استعلام عمده
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {related.map((p) => (
                <div key={p.id} onClick={() => navigate(`/product/${p.id}`)} className="bg-white rounded-3xl border p-4 hover:shadow-xl hover:border-paprika-200 transition cursor-pointer">
                  ...
                  <p className="text-xs text-stone-500 mt-1">{formatPrice(p.price)}</p>
                </div>
              ))}
            </div>
          </div>
